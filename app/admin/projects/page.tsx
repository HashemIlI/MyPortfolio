...
export default function ProjectsAdminPage() {
  ...
  async function load() {
    try {
      ...
      setCategoryGroups(Array.isArray(groupData.groups) ? groupData.groups : []);
    } catch (error) {
      setProjects([]);
      toast({
  ...
  const homepageGroups = buildProjectDisplayGroups(projects, categoryGroups, {
    ...
    ...group,
    homepagePickCount: group.projects.filter((project) => project.featuredOnHomepage).slice(0, 3).length,
  }));
    ...
    <div className="max-w-6xl p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {homepageGroups.map((group) => (
          <div key={group.id} className="admin-card rounded-2xl p-4">
